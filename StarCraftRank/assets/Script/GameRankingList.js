...
import SmallRankItem from 'SmallRankItem'
cc.Class({
    ...
    start() {
        ...
        this.goal.node.active = false;
        this.smallGoal.node.active = false;
        ...
        if (CC_WECHATGAME) {
            window.wx.onMessage(data => {
                ...
                if (data.messageType == 0) {//移除排行榜
                    ...
                    this.showSmallGoal(data.MAIN_MENU_NUM, data.score);
    ...
    showSmallGoal(MAIN_MENU_NUM, score){
        if (CC_WECHATGAME) {
            var _this = this
            wx.getUserInfo({
                ...
                success: (userRes) => {
                    ...
                    wx.getFriendCloudStorage({
                        keyList: [MAIN_MENU_NUM],
                        success: res => {
                            console.log("showSmallGoal success", res);
                            let data = res.data;
                            ...
                            _this.showBattleNextOne(data, score);
                        },
                        fail: res => {
                            console.log("wx showSmallGaol fail", res);
    ...
    showNextOne(data, userData, isIphoneX){
        var nextWhomIndex = -1;
        for(let i = 0; i < data.length; i++){
            if(data[i].avatarUrl == userData.avatarUrl && i > 0)
            {
                nextWhomIndex = i - 1;
                break;
            }
        }
        if(nextWhomIndex == -1) nextWhomIndex = data.length - 1
        this.goal.init(nextWhomIndex + 1, data[nextWhomIndex])
        if(isIphoneX)
        {
            this.goal.node.setScale(cc.v2(0.75, 1))
        }
    },

    showBattleNextOne(data, score){
        this.smallGoal.node.active = true;
        var nextWhomIndex = -1;

        for(let i = 0; i < data.length; i++){
            if(data[i].KVDataList[0].value > score)
            {
                nextWhomIndex = i - 1;
            }
        }
        ...
        console.log("fffffffff", this.smallGoal.node)
        this.smallGoal.init(nextWhomIndex + 1, data[nextWhomIndex])
    },
                ...
                success: (userRes) => {
                    ...
                    wx.getFriendCloudStorage({
                        ...
                        success: res => {
                            ...
                            this.showNextOne(data, userData, isIphoneX);
                        },
                        fail: res => {
